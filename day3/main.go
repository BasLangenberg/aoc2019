...
func parseCoords(wire string) coords {
	var crds coords
	...
	for _, instruction := range strings.Split(wire, ",") {
		pos, _ := strconv.Atoi(instruction[1:])
		switch instruction[0] {
		case 'R':
			for index := 0; index < pos; index++ {
				cur.x++
				crds = append(crds, cur)
			}
